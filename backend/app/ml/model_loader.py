import os
from ultralytics import YOLO
import logging

logger = logging.getLogger(__name__)

class MLManager:
    def __init__(self):
        self.model = None
        self.model_path = os.path.join(os.path.dirname(__file__), 'best.pt')
        self.model_name = "Positive PPE Prototype"
        self.device = "cpu"
        
    def load_model(self):
        try:
            if not os.path.exists(self.model_path):
                logger.error(f"Failed to load model: file not found at {self.model_path}")
                raise FileNotFoundError(f"Model file not found at {self.model_path}")
            
            logger.info(f"Loading {self.model_name} model from {self.model_path}...")
            self.model = YOLO(self.model_path)
            self.device = self.model.device.type if hasattr(self.model, 'device') else "cpu"
            logger.info(f"Model loaded successfully on {self.device}")
        except Exception as e:
            logger.error(f"Failed to load model: {str(e)}")
            self.model = None
            raise

    def get_model_info(self):
        if not self.model:
            return {"status": "unavailable"}
            
        return {
            "model_name": self.model_name,
            "model_type": "Ultralytics YOLO (Construction PPE Positive-Only)",
            "supported_classes": list(self.model.names.values()) if hasattr(self.model, "names") else [],
            "inference_device": self.device,
            "model_status": "loaded",
            "ppe_capable": True
        }

ml_manager = MLManager()
